...
# MStallworth, 2020-Sep-07, Added code to select_cd function
# MStallworth, 2020-Sep-07, Added code to add_track function
#------------------------------------------#
...
class DataProcessor:
    ...
    def select_cd(table: list, cd_idx: int) -> DC.CD:
        ...
        try:
            # error handling to make sure ID is an integer 
            cd_idx = int(cd_idx)
        except:
            raise Exception('ID must be an integer!')
        for row in table:
            if row.cd_id == cd_idx:
                return row
        raise Exception('This CD is not in the list!')
    ...
    def add_track(track_info: tuple, cd: DC.CD) -> None:
        ...
        try:
            # error handling to make sure position is an integer
            trackPos = int(trackPos)
        except: 
            raise Exception('Track position must be an integer!')
        new_track = DC.Track(trackPos, trackTitle, trackLength)
