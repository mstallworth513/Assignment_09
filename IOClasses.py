...
# MStallworth, 2020-Sep-07, Modified code to accept a list of file names
# MStallworth, 2020-Sep-07, Added code for saving and loading track data
#------------------------------------------#
...
class FileIO:
    ...
    def load_inventory(file_name: list) -> list:
        ...
        lst_Inventory = []
        # Turns files into a list of file names
        file_name_CD = file_name[0]
        file_name_track = file_name[1]
        # TODone modify method to accept a list of file names
        try:
            # Reading CD data from text file into inventory
            ...
            # Reading track data from text file to inventory
            with open(file_name_track, 'r') as file:
                for line in file:
                    data = line.strip().split(',')
                    cd_choice = PC.DataProcessor.select_cd(lst_Inventory, int(data[0]))
                    track = DC.Track(int(data[1]), data[2], data[3])
                    cd_choice.add_track(track) 
            print('\nThese files have been loaded successfully!\n')
        except Exception as e:
            print('There was a general error!', e, e.__doc__, type(e), sep='\n')
